...
namespace CookieConsent.Service
{
    public class ConsentService
    {
        ...
        public string RenderNotificationHtml(ConsentSettings settings, string htmlContent, string culture)
        {
            var cookieContent = _storage.Read(COOKIE_KEY);

            return string.IsNullOrEmpty(cookieContent) ? GenerateConsentHtml(settings, htmlContent, culture) : string.Empty;
        }

        private string GenerateConsentHtml(ConsentSettings settings, string htmlContent, string culture)
        {
